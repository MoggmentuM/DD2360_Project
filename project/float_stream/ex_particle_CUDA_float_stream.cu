 ...
 * param12: Nfr
 *****************************/
__global__ void likelihood_kernel(double * arrayX, double * arrayY, double * xj, double * yj, double * CDF, int * ind, int * objxy, double * likelihood, unsigned char * I, double * u, double * weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int *seed, int offset,int segment_size,int i_streams) {
    int block_id = blockIdx.x;
    int i = blockDim.x * block_id + threadIdx.x + offset;
    //int i = blockDim.x * block_id + threadIdx.x;
    int y;
 ...
 */
void particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles,int N_STREAMS) {
    int max_size = IszX * IszY*Nfr;
    ...
    for (int i = 0; i < N_STREAMS; i++) 
	{
        ...
        likelihood_kernel << < num_blocks1, threads_per_block,0,streams[i]>> > (arrayX_GPU, arrayY_GPU, xj_GPU, yj_GPU, CDF_GPU, ind_GPU, objxy_GPU, likelihood_GPU, I_GPU, u_GPU, weights_GPU, Nparticles, countOnes, max_size, k, IszY, Nfr, seed_GPU,offset,SEGMENT_SIZE,i);
    }
    cudaDeviceSynchronize();
    sum1 << < num_blocks2, threads_per_block>> > (weights,partial_sums, Nparticles);
